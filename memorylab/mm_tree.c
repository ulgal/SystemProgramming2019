...
#define GET_NODE_SIZE(bp) ((GET((char *)(bp)-4)) & ~0x7)
#define GET_PARENT(bp) (GET(bp))
#define GET_LEFT_CHILD(bp) (GET((char *)(bp) + 4))
#define GET_RIGHT_CHILD(bp) (GET((char *)(bp) + 8))
#define GET_LEFT_HEIGHT(bp) (GET((char *)(bp) + 12))
#define GET_RIGHT_HEIGHT(bp) (GET((char *)(bp) + 16))
...
/* Set macro */
#define SET_NODE_SIZE(bp, val) (PUT(HEADER_ADDR(bp), val))
#define SET_PARENT(bp, val) (PUT(PARENT_ADDR(bp), val))
#define SET_LEFT_CHILD(bp, val) (PUT(LEFT_CHILD_ADDR(bp), val))
#define SET_RIGHT_CHILD(bp, val) (PUT(RIGHT_CHILD_ADDR(bp), val))
#define SET_LEFT_HEIGHT(bp, val) (PUT(LEFT_HEIGHT_ADDR(bp), val))
#define SET_RIGHT_HEIGHT(bp, val) (PUT(RIGHT_HEIGHT_ADDR(bp), val))

/* has? macro */
#define HAS_LEFT_CHILD(bp) ((GET_LEFT_CHILD(bp) != NULL) ? 1 : 0)
#define HAS_RIGHT_CHILD(bp) ((GET_RIGHT_CHILD(bp) != NULL) ? 1 : 0)
#define HAS_CHILD(bp) ((HAS_LEFT_CHILD(bp) == NULL && HAS_RIGHT_CHILD(bp) == NULL) ? 0 : 1)
#define HAS_PARENT(bp) ((GET_PARENT(bp) != NULL) ? 1 : 0)
...
static void set_tree(void *target)
{
    ...
    if (GET_RIGHT_CHILD(target) != NULL)
    {
...
static void add_node(void *bp)
{
    ...
    while (1)
    {
        ...
        {
            if (GET_LEFT_CHILD(cur_node) != NULL)
            {
        ...
        {
            if (GET_RIGHT_CHILD(cur_node) != NULL)
            {
...
{
    /* YOUR IMPLEMENTATION */
    if ((tree_dummy_header = mem_sbrk(6 * WSIZE)) == (void *)-1)
    {
        return 1;
    }
    PUT(tree_dummy_header, 0);                                /* Alignment padding */
    PUT(tree_dummy_header + (1 * WSIZE), PACK(2 * DSIZE, 1)); /* Prologue padding */
    PUT(tree_dummy_header + (2 * WSIZE), NULL);               /* tree dummy header */
    PUT(tree_dummy_header + (3 * WSIZE), NULL);               /* garbage */
    PUT(tree_dummy_header + (4 * WSIZE), PACK(2 * DSIZE, 1)); /* Prologue footer */
    PUT(tree_dummy_header + (5 * WSIZE), PACK(0, 1));         /* Epilogue header */
    tree_dummy_header += (2 * WSIZE);
